...
def _sso_login(page, base_url, username=_IDP_USERNAME, password=_IDP_PASSWORD):
    """Walk through the full SSO flow starting from /login. Caller owns the context."""
    page.goto(f"{base_url}/login")
...
def test_sso_button_present_on_login_page(browser, browser_context_args, base_url):
...
def test_sso_invalid_credentials_stays_on_idp(browser, browser_context_args, base_url):
...
def test_sso_logout_clears_session(browser, browser_context_args, base_url):
...
def test_sso_next_param_redirects_after_login(browser, browser_context_args, base_url):
...
def test_sso_admin_user_has_staff_access(browser, browser_context_args, base_url):
...
def test_sso_when_already_authenticated_preserves_session(browser, browser_context_args, base_url):
